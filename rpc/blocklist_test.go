...
8.8.8.8`
	...
	// Check that all are single IP ranges
	expectedIPs := []string{"192.168.1.100", "10.0.0.1", "8.8.8.8"}
	for i, expectedIP := range expectedIPs {
		if !ranges[i].start.Equal(net.ParseIP(expectedIP)) {
			t.Errorf("Range %d start: expected %s, got %s", i, expectedIP, ranges[i].start)
		}
		if !ranges[i].end.Equal(net.ParseIP(expectedIP)) {
			t.Errorf("Range %d end: expected %s, got %s", i, expectedIP, ranges[i].end)
		}
	}
}
...
10.0.0.0/8`
	...
	// Test the 192.168.1.0/24 range
	if !ranges[0].start.Equal(net.ParseIP("192.168.1.0")) {
		t.Errorf("First range start: expected 192.168.1.0, got %s", ranges[0].start)
	}
	if !ranges[0].end.Equal(net.ParseIP("192.168.1.255")) {
		t.Errorf("First range end: expected 192.168.1.255, got %s", ranges[0].end)
	}
}
...
Bad Peer Network:10.0.0.1-10.0.0.100`
	...
	if !ranges[0].start.Equal(net.ParseIP("192.168.1.1")) {
		t.Errorf("First range start: expected 192.168.1.1, got %s", ranges[0].start)
	}
	if !ranges[0].end.Equal(net.ParseIP("192.168.1.255")) {
		t.Errorf("First range end: expected 192.168.1.255, got %s", ranges[0].end)
	}
	...
	// Test second range
	if !ranges[1].start.Equal(net.ParseIP("10.0.0.1")) {
		t.Errorf("Second range start: expected 10.0.0.1, got %s", ranges[1].start)
	}
	if !ranges[1].end.Equal(net.ParseIP("10.0.0.100")) {
		t.Errorf("Second range end: expected 10.0.0.100, got %s", ranges[1].end)
	}
}
...
func TestBlocklistManager_Update_NotModified(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Check for caching headers on second request
		if requestCount > 1 {
